      ...
      <GlassCard style={{ marginBottom: 12 }}>
        {items.map((item, i) => (
          <div key={item.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: i < items.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
            <div>
              <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 14, fontWeight: 600 }}>{item.label}</div>
              <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 12, marginTop: 2 }}>{item.desc}</div>
            </div>
    ...
    <div>
      <GlassCard style={{ marginBottom: 12 }}>
        <div style={{ marginBottom: 20 }}>
          <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 14, fontWeight: 600, marginBottom: 10 }}>Color de acento</div>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 14, fontWeight: 600, marginBottom: 10 }}>Tamaño de texto</div>
          <div style={{ display: 'flex', gap: 8 }}>
            {['small', 'normal', 'large'].map(size => (
              <button key={size} onClick={() => setFontSize(size)}
                style={{ flex: 1, padding: '8px', borderRadius: 10, border: 'none', cursor: 'pointer', background: fontSize === size ? 'linear-gradient(135deg,#7c3aed,#06b6d4)' : 'rgba(255,255,255,0.06)', color: 'rgba(240,240,255,0.9)', fontSize: size === 'small' ? 11 : size === 'normal' ? 13 : 15, fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 14, fontWeight: 600 }}>Animaciones</div>
            <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 12, marginTop: 2 }}>Transiciones y efectos visuales</div>
          </div>
      ...
      <GlassCard style={{ marginBottom: 12, textAlign: 'center', padding: '20px' }}>
        <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 11, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Vista previa</div>
        <div style={{ color: accent, fontSize: 18, fontWeight: 800, marginBottom: 4 }}>Kronos</div>
        <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: fontSize === 'small' ? 12 : fontSize === 'large' ? 16 : 14 }}>Así se verá el texto en la app</div>
      </GlassCard>
...
function AccountTab() {
  ...
  return (
    <div>
      <GlassCard style={{ marginBottom: 12 }}>
        <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 11, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>Información de cuenta</div>
        {[
      ...
      <button onClick={handleExportData}
        style={{ width: '100%', padding: '13px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(79,172,254,0.2)', color: 'rgba(240,240,255,0.9)', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 10 }}>
        📦 Exportar mis datos
      </button>
      ...
      {!confirmDelete ? (
        ...
        <GlassCard style={{ border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.06)' }}>
          ...
          <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 12, marginBottom: 12 }}>
          ...
          <input value={deleteInput} onChange={e => setDeleteInput(e.target.value)} placeholder="Escribe ELIMINAR"
            style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '10px 14px', color: 'rgba(240,240,255,0.9)', fontSize: 14, outline: 'none', marginBottom: 10, boxSizing: 'border-box', fontFamily: 'inherit' }} />
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => { setConfirmDelete(false); setDeleteInput(''); }}
              style={{ flex: 1, padding: '10px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: 'none', color: 'rgba(240,240,255,0.9)', cursor: 'pointer', fontSize: 13 }}>
              Cancelar
            </button>
    ...
    <div style={{ minHeight: '100vh', background: '#ffffff', paddingBottom: 100 }}>
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '16px' }}>
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 13, marginTop: 2 }}>Cuenta, privacidad y apariencia</div>
        </div>
