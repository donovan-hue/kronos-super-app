    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <GlassCard style={{ width: '100%', maxWidth: 480, borderRadius: '20px 20px 0 0', paddingBottom: 32, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 18, fontWeight: 700 }}>Nueva Reservación</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'rgba(240,240,255,0.45)', fontSize: 22, cursor: 'pointer', lineHeight: 1 }}>✕</button>
        </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <button type="button"
                  ...
                  style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,0,200,0.15)', color: 'rgba(240,240,255,0.9)', fontSize: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  −
                </button>
                <span style={{ color: 'rgba(240,240,255,0.9)', fontSize: 22, fontWeight: 700, minWidth: 32, textAlign: 'center' }}>{form.partySize}</span>
                <button type="button"
                  onClick={() => set('partySize', Math.min(20, form.partySize + 1))}
                  style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,0,200,0.15)', color: 'rgba(240,240,255,0.9)', fontSize: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  +
                </button>
                <span style={{ color: 'rgba(240,240,255,0.45)', fontSize: 13 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
            <span style={{ color: 'rgba(240,240,255,0.9)', fontWeight: 700, fontSize: 15 }}>{reservation.businessName}</span>
            <StatusBadge status={reservation.status} />
          </div>

          <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 11, marginBottom: 8 }}>
            {businessTypeLabel(reservation.businessType)}
          </div>

          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'rgba(240,240,255,0.45)', fontSize: 13 }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 20 }}>
            {[
              ...
              <GlassCard key={s.label} style={{ textAlign: 'center', padding: '14px 10px' }}>
                ...
                <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 10, letterSpacing: 0.5 }}>{s.label}</div>
              </GlassCard>
            ))}
          </div>
        )}
        ...
        {loading ? (
          ...
          <GlassCard style={{ textAlign: 'center', padding: '50px 24px' }}>
            ...
            <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 17, fontWeight: 700, marginBottom: 6 }}>
              {filter === 'all' ? 'Sin reservaciones' : `Sin reservaciones ${FILTERS.find(f => f.id === filter)?.label.toLowerCase()}`}
            </div>
            <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 13, marginBottom: 24 }}>
