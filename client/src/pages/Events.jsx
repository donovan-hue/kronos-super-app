...
function QRDisplay({ value, size = 200 }) {
  ...
  return (
    <div style={{ background: 'transparent', padding: 12, borderRadius: 12, display: 'inline-block', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
      ...
      <div style={{ textAlign: 'center', fontSize: 10, color: 'rgba(201,206,212,0.40)', marginTop: 4, fontFamily: 'monospace', letterSpacing: 1 }}>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(201,206,212,0.50)', backdropFilter: 'blur(6px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: 'transparent', borderRadius: 20, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              <div style={{ fontSize: 11, color: 'rgba(201,206,212,0.50)', marginBottom: 4 }}>Fecha inicio *</div>
              <input style={inp} type="datetime-local" value={form.startDate} onChange={e => set('startDate', e.target.value)} required />
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'rgba(201,206,212,0.50)', marginBottom: 4 }}>Fecha fin</div>
          ...
          <div>
            ...
            {form.ticketTypes.map((tt, i) => (
              <div key={i} style={{ background: 'rgba(190,200,212,0.04)', border: '1px solid rgba(190,200,212,0.12)', borderRadius: 12, padding: 12, marginBottom: 8 }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(201,206,212,0.60)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: 'transparent', borderRadius: 24, width: '100%', maxWidth: 400, padding: 28, textAlign: 'center' }}>
        ...
        <div style={{ fontWeight: 700, fontSize: 16, color: '#c9ced4', marginBottom: 4 }}>{event.title}</div>
        <div style={{ color: 'rgba(201,206,212,0.50)', fontSize: 13, marginBottom: 20 }}>{fmt(event.startDate)} · {fmtTime(event.startDate)}</div>

        {loading ? (
          <div style={{ color: 'rgba(201,206,212,0.40)', padding: 24 }}>Cargando...</div>
        ) : ticket ? (
          <div>
          ...
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, color: 'rgba(201,206,212,0.50)', marginBottom: 8, textAlign: 'left' }}>Tipo de boleto:</div>
              ...
              <div style={{ fontSize: 11, color: 'rgba(201,206,212,0.40)', marginTop: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, marginBottom: 12 }}>
              {CATEGORIES.map(c => (
                <button key={c} onClick={() => setCategory(c)} style={{
                  ...
                  background: category === c ? 'linear-gradient(180deg,#2c2f32 0%,#1a1c1e 100%)' : 'rgba(190,200,212,0.06)',
                  color: category === c ? '#fff' : 'rgba(201,206,212,0.60)',
                  transition: 'all 0.2s',
    ...
    <GlassCard style={{ padding: 0, overflow: 'hidden', cursor: 'pointer' }} onClick={onSelect}>
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 15, background: 'linear-gradient(180deg,#2c2f32 0%,#1a1c1e 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
