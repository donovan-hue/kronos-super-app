...
export default function CommunityDetail() {
  ...
  if (!community) {
    return (
      <div style={{ minHeight: '100vh', background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(201,206,212,0.50)' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 16px' }}>
        ...
        <GlassCard style={{ marginTop: -30, marginBottom: 16, position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ color: '#c9ced4', fontSize: 20, fontWeight: 800, marginBottom: 4 }}>{community.name}</div>
            ...
            <button onClick={handleJoin}
              style={{ padding: '8px 20px', borderRadius: 20, fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer', background: isMember ? 'rgba(190,200,212,0.07)' : 'linear-gradient(180deg,#2c2f32 0%,#1a1c1e 100%)', color: '#fff' }}>
            ...
            <GlassCard style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={handlePost} disabled={posting || !postText.trim()}
                  style={{ padding: '8px 24px', borderRadius: 20, background: postText.trim() ? 'linear-gradient(180deg,#2c2f32 0%,#1a1c1e 100%)' : 'rgba(255,255,255,0.08)', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, cursor: postText.trim() ? 'pointer' : 'default' }}>
                  ...
                  return (
                    <GlassCard key={post._id}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                        ...
                        />
                        <div style={{ flex: 1 }}>
                          <div style={{ color: '#c9ced4', fontSize: 13, fontWeight: 600 }}>{post.author?.firstName} {post.author?.lastName || post.author?.username}</div>
                          <div style={{ color: 'rgba(201,206,212,0.35)', fontSize: 11 }}>{new Date(post.createdAt).toLocaleDateString('es-MX', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</div>
                        </div>
                          ...
                          {post.comments?.slice(-3).map((c, i) => (
                            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', marginBottom: 8 }}>
                              ...
                              <div style={{ background: 'rgba(79,172,254,0.07)', borderRadius: 10, padding: '6px 10px', flex: 1 }}>
                                ...
                                <div style={{ color: 'rgba(201,206,212,0.65)', fontSize: 12 }}>{c.text}</div>
                              </div>
                            </div>
                          ))}
              ...
              return (
                <GlassCard key={memberId}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    />
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#c9ced4', fontSize: 14, fontWeight: 600 }}>{member.firstName} {member.lastName || member.username}</div>
                      <div style={{ color: 'rgba(201,206,212,0.50)', fontSize: 12 }}>@{member.username}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {roleBadge && (
                        <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: roleBadge.bg, color: '#c9ced4', fontWeight: 700 }}>
                          {roleBadge.label}
                        </span>
                      )}
