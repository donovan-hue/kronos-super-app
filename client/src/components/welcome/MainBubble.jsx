...
const MainBubble = ({ onGetStarted, onExplore, onSocialLogin }) => {
  const handleSocialLogin = (provider) => {
    ...
    if (provider === 'google' || provider === 'facebook') {
      window.location.assign(`${API_ORIGIN}/api/auth/${provider}`);
      return;
    }
    window.alert('El inicio de sesión con Apple todavía no está disponible. Usa Google, Facebook o correo electrónico.');
  };
  useEffect(() => {
    ...
    <div style={styles.wrapper}>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.socialRow}>
          {[
            { provider: 'apple', icon: <FaApple size={20} /> },
            { provider: 'google', icon: <FaGoogle size={18} /> },
            { provider: 'facebook', icon: <FaFacebookF size={18} /> },
          ].map(({ provider, icon }) => (
            <button key={provider} type="button" onClick={() => handleSocialLogin(provider)} aria-label={`Continuar con ${provider}`} style={styles.socialBtn}>{icon}</button>
          ))}
        </div>
      </div>
    </div>
