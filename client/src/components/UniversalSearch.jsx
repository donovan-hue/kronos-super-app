        ...
        <form onSubmit={handleSubmit} style={{ position: 'relative', marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ flex: 1, position: 'relative' }}>
              <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', fontSize: 16, color: 'rgba(240,240,255,0.45)' }}>🔍</span>
              <input
                ref={inputRef}
                value={query}
                ...
                autoComplete="off"
                style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,0,200,0.15)', borderRadius: 28, padding: '11px 40px 11px 42px', color: 'rgba(240,240,255,0.9)', fontSize: 14, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
              />
              {query && (
                <button type="button" onClick={clearSearch}
                  style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'rgba(240,240,255,0.45)', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>
                  ✕
                </button>
              )}
            </div>
          ...
          {showDrop && suggestions.length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: 'rgba(255,255,255,0.98)', border: '1.5px solid rgba(79,172,254,0.2)', borderRadius: 16, marginTop: 6, overflow: 'hidden', boxShadow: '0 8px 32px rgba(255,255,255,0.05)' }}>
              {suggestions.map((s, i) => (
                <button key={i} type="button"
                  onClick={() => { setQuery(s.text); setShowDrop(false); doSearch(s.text, category); setSearchParams({ q: s.text }); }}
                  style={{ width: '100%', textAlign: 'left', padding: '10px 16px', background: 'none', border: 'none', color: 'rgba(240,240,255,0.9)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 10, borderBottom: i < suggestions.length - 1 ? '1px solid rgba(79,172,254,0.08)' : 'none' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, overflowX: 'auto', paddingBottom: 2 }}>
          {CATEGORIES.map(cat => (
            <button key={cat.value} onClick={() => handleCategoryChange(cat.value)}
              style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, border: '1.5px solid', borderColor: category === cat.value ? 'transparent' : 'rgba(79,172,254,0.2)', cursor: 'pointer', background: category === cat.value ? 'linear-gradient(135deg,#4facfe,#f3a0ff)' : 'rgba(255,255,255,0.8)', color: category === cat.value ? '#fff' : 'rgba(240,240,255,0.45)', transition: 'all 0.2s' }}>
        ...
        {!loading && results && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            {results.posts?.length > 0 && (category === 'all' || category === 'posts') && (
              <section>
                <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>
                  ...
                  {results.posts.map(post => (
                    <GlassCard key={post._id} style={{ padding: '12px 14px', cursor: 'pointer' }}
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 6 }}>
                        ...
                        <div>
                          <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 13, fontWeight: 600 }}>{post.author?.firstName || post.author?.username}</div>
                          <div style={{ color: 'rgba(240,240,255,0.45)', fontSize: 11 }}>@{post.author?.username}</div>
                        </div>
                      </div>
                  ...
                  {results.users.map(u => (
                    <GlassCard key={u._id} style={{ padding: '12px 14px', cursor: 'pointer' }}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 14, fontWeight: 600 }}>{u.firstName} {u.lastName}</div>
                  ...
                  {results.products.map(p => (
                    <GlassCard key={p._id} style={{ padding: 0, overflow: 'hidden', cursor: 'pointer' }}
                      ...
                      <div style={{ padding: '10px 12px' }}>
                        <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
