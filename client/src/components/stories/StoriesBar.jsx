...
function StoryRing({ group, onOpen, isMine }) {
  ...
  const ringStyle = hasUnviewed
    ...
    : { background: 'rgba(240,240,255,0.45)', padding: 2 };
  ...
  return (
    ...
    >
      ...
      <span style={{ color: 'rgba(240,240,255,0.45)', fontSize: 10, maxWidth: 60, textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
function AddStoryButton({ onAdd }) {
  return (
    ...
    >
      ...
      <span style={{ color: 'rgba(240,240,255,0.45)', fontSize: 10 }}>Añadir</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 32, left: 16, display: 'flex', alignItems: 'center', gap: 10, zIndex: 1 }}>
        ...
        />
        <div>
          <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 13, fontWeight: 700 }}>{group.author.firstName} {group.author.lastName}</div>
      ...
      <div style={{ position: 'absolute', top: 32, right: 16, display: 'flex', gap: 8, zIndex: 1 }}>
        {isMyStory && (
          <button onClick={handleDeleteStory}
            style={{ background: 'rgba(239,68,68,0.3)', border: '1px solid rgba(239,68,68,0.5)', color: 'rgba(240,240,255,0.9)', fontSize: 14, cursor: 'pointer', borderRadius: 20, padding: '4px 10px', backdropFilter: 'blur(4px)' }}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9998, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#13131f', border: '1px solid rgba(124,58,237,0.3)', borderRadius: 20, padding: 28, width: '90%', maxWidth: 380 }}>
        <div style={{ color: 'rgba(240,240,255,0.9)', fontSize: 18, fontWeight: 700, marginBottom: 20 }}>Nueva Historia</div>
        ...
        <textarea
          ...
          maxLength={300}
          style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,0,200,0.15)', borderRadius: 10, padding: '10px 14px', color: 'rgba(240,240,255,0.9)', fontSize: 14, resize: 'none', outline: 'none', minHeight: 60, fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: 12 }}
        />
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'rgba(240,240,255,0.45)', fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
