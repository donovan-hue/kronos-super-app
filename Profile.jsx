...
function Profile() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (isOwnProfile) {
          ...
          setCoverPreview(profileData.cover || '');
        }
  ...
  // Manejo de archivos de imagen para el Modal
  const handleFileChange = (e, type) => {
    const file = e.target.files[0];
    if (file) {
      setEditData(prev => ({ ...prev, [type === 'avatar' ? 'avatarFile' : 'coverFile']: file }));
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#ffffff', paddingBottom: 100 }}>
      ...
      {/* Cover + Avatar */}
      <div style={{ position: 'relative', height: 180, background: profile.cover ? `url(${profile.cover}) center/cover no-repeat` : 'linear-gradient(135deg,rgba(79,172,254,0.2),rgba(0,242,254,0.15),rgba(243,160,255,0.18))', overflow: 'hidden' }}>
        {!profile.cover && (
      ...
      {/* MODAL DE EDICIÓN DE PERFIL FLOTANTE */}
      {showEditModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <GlassCard style={{ width: '100%', maxWidth: 440, padding: 24, borderRadius: 20, background: '#ffffff', color: '#0a0a14' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>Editar Perfil</h3>
            ...
            <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 4, color: 'rgba(10,10,20,0.6)' }}>Foto Perfil</label>
                  <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'avatar')} style={{ fontSize: 11, width: '100%' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 4, color: 'rgba(10,10,20,0.6)' }}>Portada</label>
                  <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'cover')} style={{ fontSize: 11, width: '100%' }} />
                </div>
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 700, display: 'block', marginBottom: 4, color: 'rgba(10,10,20,0.7)' }}>Nombre</label>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, display: 'block', marginBottom: 4, color: 'rgba(10,10,20,0.7)' }}>Apellido</label>
                <input type="text" value={editData.lastName} onChange={(e) => setEditData({ ...editData, lastName: e.target.value })}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(10,10,20,0.15)', fontSize: 13, boxSizing: 'border-box' }} />
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 700, display: 'block', marginBottom: 4, color: 'rgba(10,10,20,0.7)' }}>Biografía</label>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 10 }}>
                <button type="button" onClick={() => setShowEditModal(false)}
                  style={{ padding: '9px 18px', borderRadius: 20, border: 'none', background: 'rgba(10,10,20,0.06)', color: '#0a0a14', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                  Cancelar
                </button>
                <button type="submit" disabled={savingProfile}
                  style={{ padding: '9px 22px', borderRadius: 20, border: 'none', background: 'linear-gradient(135deg,rgba(124,58,237,0.8),rgba(168,85,247,0.8))', color: '#ffffff', fontWeight: 700, fontSize: 13, cursor: 'pointer', opacity: savingProfile ? 0.6 : 1 }}>
