...
const kronosOrchestrator = require('./orchestrator');
...
// Iniciar servidor solo cuando este archivo se ejecuta directamente. Al
// importarlo (por ejemplo, desde Supertest), se exporta la aplicación sin
// abrir un puerto; así las pruebas pueden gestionar su propio ciclo de vida.
...
if (require.main === module) {
  ...
  server.listen(PORT, () => {
    ...
    try {
      kronosOrchestrator.initialize();
      console.log("✓ KRONOS ORCHESTRATOR ONLINE");
    } catch (err) {
      console.error("✗ Error iniciando KRONOS ORCHESTRATOR:", err.message);
    }
  });
}
